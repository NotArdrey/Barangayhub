 ...
 * @param string $action      A short identifier for the action (e.g., "ACCESS DASHBOARD").
...
// Log the access of the dashboard page.
logAuditTrail($pdo, $_SESSION['user_id'], "ACCESS DASHBOARD", "Dashboard", null, "Barangay Admin accessed the dashboard page.");

// Get Barangay Admin's barangay name from the session.
...
// ----------------------
// Query Audit Trail records for current admin with admin name
// ----------------------
...
");
$auditStmt->execute([':barangay_id' => $barangay_id]);
...
<head>
  ...
  <!-- Load Tesseract.js for OCR (for auto-fill on image upload) -->
  ...
  <aside class="fixed left-0 top-0 h-screen w-64 bg-white border-r border-gray-200 p-4 shadow-md">
    ...
    <nav aria-label="Main Navigation">
      <ul class="space-y-1">
        ...
        <li>
          <a href="#auditTrail" class="nav-link flex items-center p-3 rounded-lg hover:bg-gray-100 transition-colors group">
            <span class="icon-container p-2 rounded-lg bg-gray-100 mr-3">
              <!-- Example icon: clipboard list -->
              <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" stroke="currentColor"
                   stroke-width="1.5" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" 
            ...
            <span class="text-gray-700 font-medium">Audit Trail</span>
          </a>
        </li>
        <!-- Logout -->
        <li>
  ...
  <main class="ml-64 p-8 space-y-8">
      ...
      <div class="bg-white p-6 rounded-lg shadow">
        ...
        <form class="space-y-4">
          <div>
            ...
            <input type="text" placeholder="Enter site title" class="w-full p-2 border rounded-lg">
          </div>
          <div>
            <label class="block text-gray-700">Contact Email</label>
            <input type="email" placeholder="Enter contact email" class="w-full p-2 border rounded-lg">
          </div>
    ...
    <section id="auditTrail" class="hidden">
      <header class="mb-6 flex justify-between items-center">
        ...
        <input type="text" id="auditSearch" placeholder="Search audit trail..." class="w-1/3 p-2 border rounded">
      </header>
      <div class="bg-white p-6 rounded-lg shadow">
        <div class="overflow-x-auto">
          <table class="min-w-full border border-gray-200" id="auditTable">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-4 py-2 border-b text-left text-gray-700">Action</th>
                <th class="px-4 py-2 border-b text-left text-gray-700">Table</th>
                <th class="px-4 py-2 border-b text-left text-gray-700">Performed By</th>
                <th class="px-4 py-2 border-b text-left text-gray-700">Description</th>
                <th class="px-4 py-2 border-b text-left text-gray-700">Timestamp</th>
              </tr>
            </thead>
            <tbody>
              <?php foreach ($auditRecords as $record): ?>
              <tr class="hover:bg-gray-50">
                <td class="px-4 py-2 border-b"><?php echo htmlspecialchars($record['action']); ?></td>
                <td class="px-4 py-2 border-b"><?php echo htmlspecialchars($record['table_name']); ?></td>
                <td class="px-4 py-2 border-b"><?php echo htmlspecialchars($record['admin_name']); ?></td>
                <td class="px-4 py-2 border-b"><?php echo htmlspecialchars($record['description']); ?></td>
                <td class="px-4 py-2 border-b"><?php echo htmlspecialchars($record['action_timestamp']); ?></td>
              </tr>
              <?php endforeach; ?>
              <?php if (empty($auditRecords)): ?>
              <tr>
                <td colspan="5" class="px-4 py-2 text-center text-gray-600">No audit trail entries found.</td>
              </tr>
              <?php endif; ?>
            </tbody>
          </table>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('transcribeButton').addEventListener('click', function() {
        ...
        if (voiceInput && voiceInput.files.length > 0) {
          ...
          Tesseract.recognize(voiceInput.files[0], 'eng').then(({ data: { text } }) => {
            Swal.close();
      ...
      // History search for Document Request History section.
